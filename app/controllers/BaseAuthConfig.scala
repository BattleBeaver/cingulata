...
trait BaseAuthConfig extends AuthConfig {
  ...
  def resolveUser(id: Id)(implicit ctx: ExecutionContext) = Future.successful(Option(User("123","1","22", 1L)))
  ...
  def authorize(user: User, authority: Authority)(implicit ctx: ExecutionContext) = Future.successful((null, authority) match {
    case _ => false
  })
