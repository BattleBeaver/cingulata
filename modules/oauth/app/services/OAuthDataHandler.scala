...
class OAuthDataHandler @Inject()(userService: UserService, accessTokenService: AccessTokenService)(implicit  val executionContext: ExecutionContext) extends DataHandler[User] {
  ...
  def createAccessToken(authInfo: AuthInfo[User]): Future[AccessToken] = accessTokenService.create(authInfo)

  def getStoredAccessToken(authInfo: AuthInfo[User]): Future[Option[AccessToken]] = accessTokenService.getStoredAccessToken(authInfo)
